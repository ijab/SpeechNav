...
QDataStream &operator>>(QDataStream &stream, QList<QNetworkCookie> &list)
{
    ...
    for(quint32 i = 0; i < count; ++i)
    {
        ...
        if (newCookies.count() == 0 && value.length() != 0) {
            qWarning() << "TSCookieJar: Unable to parse saved cookie:" << value;
        }
...
TSCookieJar::TSCookieJar(QObject *parent)
    : QNetworkCookieJar(parent)
    , m_loaded(false)
    , m_saveTimer(new TSAutoSaver(this))
    , m_acceptCookies(AcceptAlways)
{
}

TSCookieJar::~TSCookieJar()
{
...
void TSCookieJar::purgeOldCookies()
{
    QList<QNetworkCookie> cookies = allCookies();
    if (cookies.isEmpty())
        return;
...
QList<QNetworkCookie> TSCookieJar::cookiesForUrl(const QUrl &url) const
{
    TSCookieJar *that = const_cast<TSCookieJar*>(this);
    if (!m_loaded)
        that->load();
...
bool TSCookieJar::setCookiesFromUrl(const QList<QNetworkCookie> &cookieList, const QUrl &url)
{
    if (!m_loaded)
        load();
...
TSCookieJar::AcceptPolicy TSCookieJar::acceptPolicy() const
{
    if (!m_loaded)
        (const_cast<TSCookieJar*>(this))->load();
    return m_acceptCookies;
}

void TSCookieJar::setAcceptPolicy(AcceptPolicy policy)
{
    if (!m_loaded)
        load();
...
TSCookieJar::KeepPolicy TSCookieJar::keepPolicy() const
{
    if (!m_loaded)
        (const_cast<TSCookieJar*>(this))->load();
...
void TSCookieJar::setKeepPolicy(KeepPolicy policy)
{
    if (!m_loaded)
        load();
...
QStringList TSCookieJar::blockedCookies() const
{
    if (!m_loaded)
        (const_cast<TSCookieJar*>(this))->load();
...
QStringList TSCookieJar::allowedCookies() const
{
    if (!m_loaded)
        (const_cast<TSCookieJar*>(this))->load();
...
QStringList TSCookieJar::allowForSessionCookies() const
{
    if (!m_loaded)
        (const_cast<TSCookieJar*>(this))->load();
...
void TSCookieJar::setBlockedCookies(const QStringList &list)
{
    if (!m_loaded)
        load();
...
void TSCookieJar::setAllowedCookies(const QStringList &list)
{
    if (!m_loaded)
        load();
...
void TSCookieJar::setAllowForSessionCookies(const QStringList &list)
{
    if (!m_loaded)
        load();
...
CookieModel::CookieModel(TSCookieJar *cookieJar, QObject *parent)
...
CookieExceptionsModel::CookieExceptionsModel(TSCookieJar *cookiejar, QObject *parent)
    : QAbstractTableModel(parent)
    , m_cookieJar(cookiejar)
{
